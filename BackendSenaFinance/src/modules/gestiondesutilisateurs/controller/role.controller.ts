...
import { checkRelationsOneToMany } from "../../../configs/checkRelationsOneToManyBeforDelete";
    ...
    export const getAllRole = async (req: Request, res: Response) => {
        ...
        try {
          const data = await myDataSource.getRepository(Role)
  ...
  */
    ...
    export const getRolesNotIn = async (req: Request, res: Response) => {
        try {
          const userId = req.params.userId;
      
          // Récupérer les rôles que l'utilisateur possède
          const userRoles = await myDataSource.getRepository(Role)
            .createQueryBuilder("role")
            .select("role.id")
          ...
          const rolesNotIn = await myDataSource.getRepository(Role)
            .createQueryBuilder("role")
          ...
          return success(res, 200, rolesNotIn, message);
        } catch (error) {
